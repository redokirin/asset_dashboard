<template>
    <div class="space-y-2">
        ...
        <div v-for="row in sorted" :key="row['代碼']"
            class="rounded-xl bg-gray-800 border border-gray-700/50 overflow-hidden">
            <!-- 主列（點擊展開） -->
            <div class="px-4 py-3 cursor-pointer hover:bg-gray-750 transition-colors" @click="toggleExpand(row['代碼'])">
                <div class="flex flex-wrap sm:flex-nowrap sm:items-center items-start gap-x-3 gap-y-1.5">
                    ...
                    <div class="w-full sm:flex-1 sm:w-auto min-w-0">
                        <div class="text-xs text-gray-400 leading-tight">
                            {{ row['市場'] }} |
                            <button class="font-mono hover:text-blue-400 hover:underline transition-colors"
                                @click.stop="emit('open-chart', { ticker: row['代碼'], name: row['名稱'] })">{{ row['代碼']
                                }}</button>
                        ...
                        <div class="flex items-center gap-2 mt-0.5">
                            ...
                            <span v-if="adv(row)?.entryZoneStatus && adv(row).entryZoneStatus !== '-'"
                                class="text-xs shrink-0">{{ adv(row).entryZoneStatus }}</span>
                        </div>
                    </div>
            ...
            <!-- 展開面板 -->
            <div v-if="expanded.has(row['代碼'])" class="border-t border-gray-700/50 bg-gray-850">
                <!-- 成本區塊（永遠顯示） -->
                ...
                <div v-if="adv(row)" class="px-4 py-3">
                    <AdvancedAnalysisPanel :res="adv(row)" :actionable="signalFor(row['代碼']).actionable"
                        :warning="signalFor(row['代碼']).warning" :holdOff="signalFor(row['代碼']).holdOff"
                ...
                <div v-if="adv(row)" class="px-4 pb-3 flex justify-end">
                    <button
                        ...
                        :disabled="copyingMap[row['代碼']] === 'loading'" @click.stop="copyTickerReport(row['代碼'])">
