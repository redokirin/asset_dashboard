<template>
  <div class="space-y-2">
    ...
    <div v-if="activeTab === 'decision'" class="space-y-2.5">
      ...
      <template v-if="hasZoneBar">
        <div class="space-y-0.5">
          <div class="relative h-4 text-[10px] text-gray-400">
            <span class="absolute" :style="`left:25%;transform:translateX(-50%)`">{{ fmt2(r.dailyUpper) }}</span>
            <span class="absolute" :style="`left:50%;transform:translateX(-50%)`">{{ fmt2(r.boundaryDailyRetest) }}</span>
            <span class="absolute" :style="`left:75%;transform:translateX(-50%)`">{{ fmt2(r.boundaryRetestSniper) }}</span>
          </div>
      ...
      <div
        v-if="r['技術診斷']"
        class="text-xs text-gray-300 bg-gray-700/30 rounded-lg px-3 py-2 leading-relaxed"
      >{{ r['技術診斷'] }}</div>
      ...
      <div
        v-if="holdOff"
        class="text-xs text-gray-400 bg-gray-700/30 rounded-lg px-3 py-2 leading-relaxed"
      >
        ⚪ 觀望
    ...
    <div v-else-if="activeTab === 'quant'" class="space-y-2">
      ...
      <div class="grid grid-cols-4 gap-2">
        <div v-for="ma in ['MA20','MA60','MA120','MA250']" :key="ma" class="rounded-lg bg-gray-700/40 px-3 py-2 text-center">
    ...
    <!-- ── FA ────────────────────────────────────────── -->
    <div v-else-if="activeTab === 'fa'" class="space-y-2">
