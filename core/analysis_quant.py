...
def calculate_buffered_entries_v2(df, ma5, ma20, ma250, current_price, rs_p10_price):
    ...
    final_daily = min(base_daily * (1 + BUFFER_PERCENT), ceiling)
    final_tech = min(base_tech * (1 + BUFFER_PERCENT), final_daily * (1 - MIN_GAP))
    final_sniper = min(base_sniper * (1 + BUFFER_PERCENT), final_tech * (1 - MIN_GAP))
